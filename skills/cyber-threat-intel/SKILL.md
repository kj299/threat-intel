...
2. **Consult sources.** Walk all 9 tiers from [references/source-matrix.md](references/source-matrix.md). Track which preferred (`[MUST]`) sources you actually drew from. Honestly mark inaccessible ones.
   2a. **Pull live feeds when available (MCP).** If the `threat-intel-mcp` tools are present in your environment, retrieve current indicators before composing the report. Call **`fetch_all_iocs`** once to query every configured feed concurrently, or a single-feed tool (`qfeeds_fetch_iocs`, `abuseipdb_fetch_blocklist`, `virustotal_fetch_iocs`, `otx_fetch_iocs`, `shodan_fetch_iocs`, `greynoise_fetch_iocs`, `anyrun_fetch_iocs`, `intel471_fetch_iocs`, `censys_fetch_iocs`, `urlhaus_fetch_iocs`, `threatfox_fetch_iocs`) for one source; use **`list_available_feeds`** first if you need to see what's configured. Then fold the results in:
