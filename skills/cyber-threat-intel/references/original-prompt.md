...
`chain_id | name | links (each: cwe_id, role primary/resultant, mitre_id, evidence, source) | enabling_conditions | ai_assist_factor (none/low/moderate/high) | break_points (each: at_link cwe_id, control, control_type preventive/detective/corrective, mapped_mitigation) | score | confidence | source`
...
- Score the chain via the standard engine (exploitability of weakest primary link, impact of terminal resultant, urgency uplift when `ai_assist_factor` ≥ moderate) and drive the break-point control into the Actions Matrix.
