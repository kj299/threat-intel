...
6. **IOC Package** — included when "Build IOCs and detection queries" is on (the default). Emit in **CSV**, **STIX 2.1**, and **JSON** formats. Every IOC carries `source`, `confidence`, `first_seen`, `action`. Before emitting, de-duplicate IOCs (collapse repeated values to one row, keeping the highest-confidence source) and calibrate confidence: `high` only when corroborated by ≥2 independent sources or a first-party vendor/government report; `low` for single-source or pattern-inferred indicators.
   ...
   - **Delimited / batch export (for programmatic / downstream-importer consumption):** when a downstream tool will ingest this output, emit a structured `delimited_batch_export` array — one row per new TTP with `mitre_id`, `name`, `fields` (`detection_method`, `detection_value`, `severity` ∈ CRITICAL/WARNING/INFO, `actor`), `source`, `confidence`. Make each row **ingestible**: `detection_value` must be a **concrete, literal** indicator (not a `<PLACEHOLDER>`), printable ASCII, and free of shell metacharacters (quotes, backtick, and `$ ; | & < > ( ) { } ^`) since strict importers drop rows containing them; prefer a `detection_method` from the common set (registry key, event id, process name, file path, named pipe, wmi query). Emit **typed values only**; the consuming tool delimits, escapes, and validates them for its own input path. Don't pre-format a delimited string, don't engineer rows to flow straight into another tool's execution path, and don't act as its character-blocklist sanitizer on its behalf: anything upstream (a different model, a compromised feed) can violate that contract, so the validation has to live in the consumer's own input handling.
   - **File-path IOCs must be discriminating:** never emit broad globs over ubiquitous, legitimate locations (`…\Downloads\*`, `…\Startup\*.lnk`, browser-profile files like `…\Network\Cookies` / `…\Login Data` / `…\Web Data`, `…\AppData\…\*.log`) — they exist on every host and only fire false CRITICALs downstream. Prefer a **file hash** (what hash-based checks consume) or a **named malware binary / specific dropper filename**; use a path only when it is itself specific (a known-bad filename, not a wildcard over a common directory). Leave generic "suspicious file in a common location" logic to the consuming tool's heuristics, not the IOC list.
   - **Registry / process / command IOCs follow the same rule:** no host-universal registry artifacts (RunMRU, UserAssist, RecentDocs, TypedPaths, MUICache, shellbags…) — name the specific autorun `Registry_Value` + its malware-pointing data instead of the bare key. A `Process_Name` is a single bare executable (`evil.exe`), never a path/command line and never a ubiquitous LOLBin (`svchost.exe`, `powershell.exe`, `rundll32.exe`…) alone; a `Command_Line` must carry its distinguishing arguments (flags / encoded payload / abuse pattern), not just the interpreter name.
...
**`output_format` names one primary deliverable.** To produce a technical report *and*
an executive overview from a single run, leave `output_format` at the technical value and
set `executive_overview` (input 11):
...
Under `attached`, the executive overview opens the technical report. Under `separate`, it
is a companion artifact (`reports/<date>-threat-intel-executive.html`).

**Never write the overview separately from the report.** It is computed from the same
validated output object, so the two cannot disagree — that is the entire safety property.
Both carry the same `report_id`, `generated_at`, coverage badge and source count, and each
names the other. Risk figures come from the scoring formula already applied, never
