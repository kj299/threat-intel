...
flowchart TD
    ...
    subgraph MCP["threat-intel-mcp (stdio transport)"]
        ...
        subgraph Adapters["Adapters"]
            ...
            Pulsedive["PulsediveAdapter\nadapters/pulsedive.py\nkey in query param\nONE request/fetch (50/day)\n60-min cache"]
    ...
    subgraph Ext["External Feeds"]
        ...
        VT_API["VirusTotal API v3\nhttps://www.virustotal.com/api/v3\nGET /ip_addresses/{ip} · /domains/{d}\nper-indicator lookup (the bulk feeds\npath was removed in #203 — it 404d)"]
        ...
        Pulsedive_API["Pulsedive Explore\nhttps://pulsedive.com/api\nGET /explore.php?q=..&key=.."]
    ...
    Resilience -->|"guarded_fetch"| Pulsedive
    ...
    Pulsedive -->|"GET /explore.php (one page)"| Pulsedive_API
    Pulsedive_API -->|"JSON results[]"| Pulsedive
    EPSS -->|"GET ?cve=... (no auth)"| EPSS_API
    EPSS_API -->|"data[] cve · epss · percentile"| EPSS
    OSV -->|"GET /vulns/{cve} (no auth)"| OSV_API
    OSV_API -->|"OSV record · affected[] · fixed"| OSV
    ...
    Pulsedive -->|"raw ioc_network objects"| Normalize
...
| MCP Server | `mcp/src/threat_intel_mcp/server.py` | FastMCP stdio server; exposes IOC tools `qfeeds_fetch_iocs`, `abuseipdb_fetch_blocklist`, `otx_fetch_iocs`, `shodan_fetch_iocs`, `greynoise_fetch_iocs`, `anyrun_fetch_iocs`, `intel471_fetch_iocs`, `censys_fetch_iocs`, `threatfox_fetch_iocs`, `fetch_all_iocs`; CVE tools `cisa_kev_fetch_cves`, `nvd_fetch_cves`, `vulncheck_fetch_cves`, `fetch_all_cves`; the enrichment tool `virustotal_enrich_iocs` (per-indicator lookup, not part of the IOC fan-out); and `list_available_feeds` |
