...
- `mcp/` -- `threat-intel-mcp` MCP server (stdio transport); runtime counterpart to the prompt skill
  - `mcp/src/threat_intel_mcp/adapters/` -- live feed adapters: Q-Feeds, AbuseIPDB, VirusTotal, AlienVault OTX (each with in-process cache + egress allowlist)
  ...
  - `mcp/src/threat_intel_mcp/server.py` -- FastMCP entry point: `fetch_all_iocs`, four single-feed tools, `list_available_feeds`
  - `mcp/tests/` -- 193 unit + httpx-mock integration tests (no live network)
